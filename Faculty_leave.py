...
cursor.execute("Create TABLE IF NOT EXISTS ADMIN(Username varchar2,Password varchar2,Security_code varchar2,email varchar2)")
...
def Faculty_registration():
    ...
    while True:
        ...
        if len(fields[2])>1:
            ...
            fields=multpasswordbox("Please Provide a valid Email Id or\ncheck internet connection",message,field_names,fields)
...
def mail_sender(email_to,subject,body):
    try:
        Email_Address='pbsiddharthaleavemanagement@gmail.com'
        Email_Password='ijllyyzrrtatooqv'
        msg=EmailMessage()
        msg['Subject']=subject
...
def updatedadminemid():
    ...
    fields_names=['Email Id:']
    fields=multenterbox(Message,Title,fields_names)
    while True:
        if fields[0]=="":
            fields=multenterbox("Emaild should not be empty",Title,fields_names)
        else:
            d=validate1(fields[0])
            if d==None or d==False:
                fields=multenterbox("Provide a valid email id or\ncheck internet connection",Title,fields_names,fields)
            elif (fields[0],) in cursor.execute('SELECT email_id from FACULTY'):
                fields=multenterbox("Email id already in use by faculty",Title,fields_names,fields)
            else:
                cursor.execute('UPDATE ADMIN SET email=?',[fields[0]])
                conn.commit()
                email_updatedadmin()
                break
def status_cleared():
...
def mv_email_updated():
    ...
    msg23=Label(root012,text="Email notification failed!\nCheck your internert",fg="green",font=('calibre',15, 'bold'),justify=CENTER)
    msg23.pack()
    ok_btn23=Button(root012,text="Ok",font=('calibre',12,'bold','underline'),bg="blue",fg='white',justify=CENTER,command=root012.destroy)
    ok_btn23.pack()
...
label2=Label(text="Faculty Leave Management System",borderwidth=13,relief=RIDGE,fg='white',bg='navy',font=('Callibri',36,"bold"),pady=3)
label2.pack(pady=203)
Adminbutn=Button(text="Admin Login",bd=13,relief=GROOVE,fg='white',bg='navy',command=adminvalidation,pady=3)
Adminbutn["font"]=Btnfont
Facultybutn=Button(text="Faculty\nRigistration",bd=13,relief=GROOVE,fg='white',bg='navy',command=Faculty_registration,pady=3)
Facultybutn["font"]=Btnfont
Faculty_forget=Button(text="Forget\nPassword",bd=13,relief=GROOVE,fg='white',bg='navy',command=Forget_Home,pady=3)
Faculty_forget["font"]=Btnfont
Facultyreg=Button(text="Faculty\nLogin",bd=13,relief=GROOVE,fg='white',bg='navy',command=Faculty_Success,pady=3)
Facultyreg["font"]=Btnfont
Exit=Button(text="Exit",bd=13,relief=GROOVE,fg='white',bg='navy',command=root.destroy,pady=3)
Exit["font"]=Btnfont
Adminbutn.place(relx = 0.5, rely = 0.4, anchor = CENTER)
Facultybutn.place(relx = 0.5, rely = 0.52, anchor = CENTER)
Faculty_forget.place(relx = 0.5, rely = 0.656, anchor = CENTER)
Facultyreg.place(relx = 0.5, rely = 0.792, anchor = CENTER)
Exit.place(relx = 0.5, rely = 0.910, anchor = CENTER)
root.mainloop()
